/*
 * Copyright ©2022 WebApp/connDb.java Powered By DZX 2022/7/11
 */
...
public class connDb {
    ...
    public static void startConn() {
        try {
            ...
            //连接数据库中间件
            try {
                con = DriverManager.getConnection("jdbc:MySQL://HadoopIP:3306/dbtaobao", "hive", "hive23ie6BYGQ8BqgPoL");
    ...
    public static ArrayList index_3() throws SQLException {
        ...
        startConn();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select cat_id,count(*) num from user_log group by cat_id order by num desc limit 5");
        while (rs.next()) {
    ...
    public static ArrayList index_4() throws SQLException {
        ...
        startConn();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select province,count(*) num from user_log group by province order by num desc");
        while (rs.next()) {
